...
namespace RssBandit.UnitTests
{
	...
	public class RssLocaterTests : CassiniHelperTestFixture
	{
		...
		/// <summary>
		/// Creates an RssLocater and scans an html page (hosted locally) 
		/// for links to Rss/Atom feeds.  Makes sure it can find the 
		/// feeds we planted in the page.
		/// </summary>
		[Test]
		public void GetRssAutoDiscoveryLinksFindsRssAtomLinks() {
			RssLocater locater = new RssLocater();
			ArrayList feeds = locater.GetRssAutoDiscoveryLinks(BASE_URL + "MultipleFeeds.htm");
			Assert.AreEqual(3, feeds.Count, "Obviously I could not count as I expected 4 feeds.");

//			ArrayList feedsExternal = locater.GetRssAutoDiscoveryLinks("http://www.dotnetmonster.com/CS/blogs/net-monster-blog/archive/2006/02/22/35.aspx");
//			Assert.AreEqual(3, feeds.Count, "Obviously I could not count as I expected 4 feeds.");
		}
