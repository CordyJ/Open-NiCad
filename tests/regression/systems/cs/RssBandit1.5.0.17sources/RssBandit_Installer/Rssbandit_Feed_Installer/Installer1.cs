...
@"<feeds refresh-rate='3600000' xmlns='http://www.25hoursaday.com/2004/RSSBandit/feeds/'>
   ...
   <feed category='Comics'>
     <title>XKCD</title>
     <link>http://xkcd.com/rss.xml</link>
   </feed>
    <feed category='Blogs'>
     <title>Robert Scoble</title>
     <link>http://scobleizer.wordpress.com/feed/</link>
   </feed> 
    <feed category='Blogs'>
   ...
   <feed category='Comics'>
    ...
    <feed category='Comics'>
     <title>Peanuts</title>
     <link>http://www.tapestrycomics.com/peanuts.xml</link>
   </feed>
   <categories>    
   ...
   <listview-layout ID='9bc6fd4047384a19a030b315ffba5723'>
    ...
    <nntp-servers>
     <server name='Microsoft'>
	  <server-address>news.microsoft.com</server-address>
	 </server>	
	</nntp-servers>
   </feeds>
";
		...
		public override void Install(IDictionary stateSaver) {
			...
			try 
			{
				...
				try {
					this.RemoveDefaultSearchFolders(); 
				}catch(Exception e){ /* Not fatal enough to halt installation process */				
					this.Context.LogMessage("RemoveDefaultSearchFolders() caused exception: " + e.Message);
				}
				try {
		...
		/// </summary>
		private void RemoveDefaultSearchFolders(){
		...
		/// </summary>
		/// <param name="savedState">An IDictionary that contains the pre-installation state of the computer. </param>
