...
namespace RssBandit.Utility.Keyboard
{
	...
	{
		private static readonly log4net.ILog _log  = Common.Logging.Log.GetLogger(typeof(ShortcutHandler));
			...
			while(reader.Read())
			{
				...
				if(nodeType == XmlNodeType.Element)
				{
					...
					else if(reader.Name == "shortcutEnumValue")
					{
						...
						{
							_log.Error("Failed to parse shortcut enum value at 'shortcutEnumValue'", e);
						}
					}
				}
				if(nodeType == XmlNodeType.EndElement && reader.Name == "shortcut")
