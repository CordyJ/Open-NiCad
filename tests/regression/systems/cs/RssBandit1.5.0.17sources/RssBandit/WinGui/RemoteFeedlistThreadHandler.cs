...
/*
 * $Id: RemoteFeedlistThreadHandler.cs,v 1.66 2007/09/17 23:54:19 carnage4life Exp $
 * Last modified by $Author: carnage4life $
 * Last modified at $Date: 2007/09/17 23:54:19 $
 * $Revision: 1.66 $
 */
#endregion
							...
							if(feedListChanged){
								...
								if(addedFeeds.Count > 0){
									...
									foreach(feedsFeed f in addedFeeds){
										...
										try{										
											int folderId = 0;
											if(!StringHelper.EmptyOrNull(f.category)){
												if(categoryTable.ContainsKey(f.category)){
													folderId = (int) categoryTable[f.category]; 
												}else{
													folderId = GetFolderId(fows, f, opmlFeedList); 
													categoryTable.Add(f.category, folderId); 
												}
											}
											sws.AddSubscription(f.link, folderId, null); 
 ...
 * Revision 1.66  2007/09/17 23:54:19  carnage4life
 * Cleaned up code and removed unnecessary steps in NewsGator syncing
 *
 * Revision 1.65  2007/09/17 23:06:44  carnage4life
 * Made optimizations in code to upload folder structure to NewsGator Online
 *
 * Revision 1.64  2007/09/17 22:10:56  carnage4life
 * Added features to support exporting folder hierarchy to NewsGator
 *
 * Revision 1.63  2007/09/17 03:10:06  carnage4life
 * no message
 *
 * Revision 1.62  2007/09/16 15:09:40  carnage4life
 * clean up
 *
 * Revision 1.61  2007/09/15 21:21:18  carnage4life
 * Sync with NewsGator now works reasonably well
 *
 * Revision 1.60  2007/09/15 19:20:26  carnage4life
 * ActiveX is now enabled by default due to embedded Flash videos
 *
