...
/*
 * $Id: LuceneIndexModifier.cs,v 1.30 2007/09/16 15:09:40 carnage4life Exp $
 * Last modified by $Author: carnage4life $
 * Last modified at $Date: 2007/09/16 15:09:40 $
 * $Revision: 1.30 $
 */
#endregion
...
namespace NewsComponents.Search
{
	...
	internal class LuceneIndexModifier: IDisposable 
	{
		...
		private bool open, flushInprogress = false, threadRunning = false, indexCorrupted=false;
		...
		{
			get { 
				if(this.indexCorrupted){ 
					this.indexBaseDirectory = Lucene.Net.Store.FSDirectory.GetDirectory(this.settings.IndexPath, true /* create */);
				}				
		...
		private void FlushPendingOperations(int batchedItemsAmount) 
		{
			try {
				...
				do {
					...
					lock (this.pendingIndexOperations.SyncRoot) {
							count = this.pendingIndexOperations.Count;
						if (count > 0) {
...
//#endif
			lock (SyncRoot)
			{
				...
				}catch(SystemException se){ /* docs out of order */ 
					_log.Error("Error adding document:", se); 

					/* see http://issues.apache.org/jira/browse/LUCENE-140 */
					indexCorrupted = true; 
				}
...
//#endif			
		...
		protected internal virtual void  CreateIndexWriter() {
			if ((this.indexWriter == null) || this.indexCorrupted) {
				...
				this.indexWriter = new IndexWriter(this.BaseDirectory, 
					LuceneSearch.GetAnalyzer(LuceneSearch.DefaultLanguage), this.indexCorrupted);
			...
			if (this.indexReader == null) {
				if (this.indexWriter != null) {
					...
					try { 
						...
						_log.Debug("Error closing index writer:", ioe); 						
					} catch(SystemException se){ /* docs out of order */ 
						_log.Debug("Error closing index writer:", se); 
						indexCorrupted = true; 
					}
 ...
 * Revision 1.30  2007/09/16 15:09:40  carnage4life
 * clean up
 *
 * Revision 1.29  2007/09/16 14:23:07  carnage4life
 * Fixed a number of Lucene related crashes
 *
 * Revision 1.28  2007/09/15 21:21:18  carnage4life
 * Sync with NewsGator now works reasonably well
 *
 * Revision 1.27  2007/09/15 18:13:45  carnage4life
 * Changes to improve performance of Lucene indexing
 *
 * Revision 1.26  2007/09/12 09:55:45  t_rendelmann
 * fixed: spelling of "deleteable"
 *
