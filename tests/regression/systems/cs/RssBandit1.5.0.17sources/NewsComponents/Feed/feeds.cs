...
/*
 * $Id: feeds.cs,v 1.33 2007/09/17 22:09:51 carnage4life Exp $
...
namespace NewsComponents.Feed {
	...
	public class feedsFeed {
		...
		/// <summary>
		/// Adds a wildcard attribute to this object's AnyAttr array. 
		/// </summary>
		/// <param name="attr">The wildcard attribute to add</param>
		public void AddWildCardAttribute(XmlAttribute attr){
			if(this.AnyAttr != null){
				XmlAttribute[] anyAttr2 = new XmlAttribute[this.AnyAttr.Length + 1]; 
				anyAttr2[0]  = attr;
				if(this.AnyAttr.Length > 0){
					this.AnyAttr.CopyTo(anyAttr2, 1); 				
				}
				this.AnyAttr = anyAttr2; 
			}else{
				this.AnyAttr = new XmlAttribute[1];
				this.AnyAttr[0] = attr; 
			}
		}
		...
		/// <summary>
		/// Gets the value of a particular wildcard attribute. If the element is not found then 
		/// null is returned
		/// </summary>
		/// <param name="namespaceUri"></param>
		/// <param name="localName"></param>
		/// <returns>The value of the wildcard element obtained by calling XmlAttribute.Value
		/// or null if the attribute is not found. </returns>
		public string GetWildCardAttributeValue(string namespaceUri, string localName){
			if(this.AnyAttr != null){
...
/*
 * $Log: feeds.cs,v $
 * Revision 1.33  2007/09/17 22:09:51  carnage4life
 * Added features to support exporting folder hierarchy to NewsGator
 *
 * Revision 1.32  2007/09/15 21:21:18  carnage4life
